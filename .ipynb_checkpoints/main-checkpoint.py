...
class VAE(nn.Module):
    def __init__(self):
        ...
        self.conv3 = nn.Conv2d(in_channels=3, out_channels=16, kernel_size=3, padding=(3-1)//2)
        self.conv4 = nn.Conv2d(in_channels=16, out_channels=1, kernel_size=3, padding=(3-1)//2)

        self.fc1 = nn.Linear(3*self.EDGE_DIM*self.EDGE_DIM, 20)
        self.fc2 = nn.Linear(3*self.EDGE_DIM*self.EDGE_DIM, 20)
        self.fc3 = nn.Linear(20, 3*self.EDGE_DIM*self.EDGE_DIM)

    def encode(self, x):
        x = F.relu(self.conv1(x))
        x = F.relu(self.conv2(x))
        x = x.view(-1, 3*self.EDGE_DIM*self.EDGE_DIM)
